...
const STRATEGIES: StrategyId[] = ["stars", "balanced", "wr-heavy", "superflex"];
...
export default function PositionBudgetBar() {
  ...
  useEffect(() => {
    ...
    // Merge: keep touched + locked, overwrite the rest.
  ...
  const max = maxBid(settings.totalBudget - planned + slots.filter((s) => !lockedSlots[s.id]).reduce((a, s) => a + valueFor(s), 0), openSlots);
  // Simpler: max bid against what's still unspent assuming current plan holds.
  const remainingForBid = settings.totalBudget - slots.filter((s) => lockedSlots[s.id]).reduce((a, s) => a + valueFor(s), 0);
  const bidCeiling = maxBid(remainingForBid, openSlots);

  const handleReset = () => {
    clearTouchedSlots();
    // Recompute against fresh state (only locks honored).
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
          ...
          <div className="flex items-center gap-2">
            <span
              ...
              title="Most you could bid on any one player and still afford $1 per remaining slot"
            >
              Max bid ${bidCeiling}
            </span>
            <span
